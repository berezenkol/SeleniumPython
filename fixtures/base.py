...
from fixtures.params import CHROME_EXECUTABLE_PATH, EXPLICIT_TIMEOUT, DOMAIN, BROWSER_TYPE
...
class BaseTestCase(unittest.TestCase):
    def get_browser(self):
        ...
        # >= 0 - if chrome not found than -1 will be returned, so if chrome found it will return 0
            webdriver.Chrome(executable_path=CHROME_EXECUTABLE_PATH)
        # elif BROWSER_TYPE.lower().find('firefox') >= 0:
        #     webdriver.Firefox(executable_path=FIREFOX_EXECUTABLE_PATH)
...
class AdminLoginTestCase(BaseTestCase):   # this class includes url open & login
    def setUp(self):
